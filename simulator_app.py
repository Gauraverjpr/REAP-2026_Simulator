...
# --- 2. INPUT LAYOUT (Form wrapper removed for dynamic UI reactivity) ---
...
with st.expander("🎓 Step 1: Academic Information", expanded=True):
    ...
    # Dynamic Fields - These will now instantly appear/disappear
...
with submit_btn_col2:
    # Changed to standard st.button since form wrapper was removed
...
if submitted:
    ...
    # --- NEW: ELIGIBILITY MESSAGES MOVED TO BOTTOM ---
    eligible_groups = []
    
    if qual_exam == "12th Board":
        if len(selected_subs) < 3:
            st.error("❌ **Action Required:** Please select exactly three distinct subjects from the 12th Board dropdowns in Step 1.")
            st.stop()
        elif len(set(selected_subs)) < 3:
            st.error("❌ **Action Required:** Please select three distinct subjects. You have selected duplicates.")
            st.stop()
        else:
            ...
            if eligible_groups:
                ...
                with st.expander("📄 Click here for more", expanded=False):
                    ...
                    * **Group 6:** Maths, two additional subjects *(Planning)*
                    ...
                    # Native Streamlit download button restored
